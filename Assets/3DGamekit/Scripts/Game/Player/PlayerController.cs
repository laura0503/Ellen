...
namespace Gamekit3D
{
    ...
    public class PlayerController : MonoBehaviour, IMessageReceiver
    {
        ...
        // Llamado automáticamente por Unity cuando el script se añade por primera vez a un gameobject o se reinicia desde el menú contextual.
        // **IMPORTANTE:** Se deja solo la asignación directa de componentes en el editor.
        void Reset()
        {
            meleeWeapon = GetComponentInChildren<MeleeWeapon>();

            // ELIMINADAS las búsquedas complejas (transform.Find y FindFirstObjectByType) de Reset()
            // para evitar el error del hilo principal. Estas se han movido a Awake().
        }

        // Llamado automáticamente por Unity cuando el script existe por primera vez en la escena.
        // **CORREGIDO:** Toda la lógica de inicialización y búsqueda de objetos ha sido movida aquí.
        void Awake()
        {
            ...
            // Asegurar que Damageable existe (Mejora de robustez de tu versión anterior)
            m_Damageable = GetComponent<Damageable>();
            if (m_Damageable == null)
            {
                m_Damageable = gameObject.AddComponent<Damageable>();
                Debug.Log("Damageable component added to Ellen");
            }

            // --- LÓGICA MOVIDA DE Reset() para evitar UnityException: GetName (hilo principal) ---

            // Si meleeWeapon no está asignado desde el editor (por Reset()), lo buscamos.
            if (meleeWeapon == null)
                meleeWeapon = GetComponentInChildren<MeleeWeapon>();
            ...
            // Búsqueda de CameraSettings (Movida de Reset)
            ...
            if (cameraSettings != null)
            {
                ...
                // CORRECCIÓN: Usar lookAt para el objetivo de la cabeza, si está disponible.
        ...
        void OnEnable()
        {
            ...
            // Obtener referencia segura a Damageable
            if (m_Damageable == null)
                m_Damageable = GetComponent<Damageable>();

            if (m_Damageable == null)
            {
                Debug.LogError("Damageable component is missing from Ellen!");
                return;
            }
        ...
        protected IEnumerator RespawnRoutine()
        {
            ...
            // Versión simple sin ScreenFader para evitar errores
        ...
        public void RespawnFinished()
        {
            ...
            if (m_Damageable == null)
            {
                ...
                if (m_Damageable == null)
                {
                    Debug.LogError("Cannot set invulnerable - Damageable is null!");
                    return;
                }
            }

            m_Damageable.isInvulnerable = false;
            Debug.Log("🛡️ Invulnerability turned OFF after respawn");
        }
        ...
        public void Die(Damageable.DamageMessage damageMessage)
        {
            ...
            if (m_Damageable != null)
                ...
                Debug.LogError("Damageable is null in Die method!");
        }
    }
}